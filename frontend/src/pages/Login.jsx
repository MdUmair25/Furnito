...
function Login() {
    ...
    const googlelogin = async () => {
        ...
        try {
            ...
            const name = user.displayName || (userEmail.includes('@') ? userEmail.split('@')[0] : 'User')
            const result = await axios.post(serverUrl + "/api/auth/googlelogin", { name, email: userEmail }, { withCredentials: true })
            await getCurrentUser()
            navigate("/")
            toast.success("Google Login Successful")
        } catch (error) {
            toast.error(error?.response?.data?.message || error?.code || error?.message || "Google Login Failed")
        } finally {
            setLoading(false)
        }
    }
            ...
            <div className="flex-1 flex flex-col items-center justify-center px-4">
                ...
                >
                    ...
                    <form onSubmit={handleLogin} className="flex flex-col gap-4">
                        ...
                        <button
                            type="submit"
                            className="w-full h-[54px] bg-blue-500 hover:bg-blue-600 active:bg-blue-700 text-white rounded-xl font-bold text-[18px] flex items-center cursor-pointer justify-center transition-all duration-200 shadow-md hover:shadow-xl"
                        >
